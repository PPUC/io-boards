...
#define UPDATE_INTERVAL_WS2812FX_BRIGHTNESS 10
    ...
    PPUC_CONTROLLER == CONTROLLER_TEENSY_OUTPUT_2 || \
    PPUC_CONTROLLER == CONTROLLER_PICO_OUTPUT
...
DMAMEM byte frameBuffer1[PPUC_NUM_LEDS_1 * ((PPUC_LED_TYPE_1 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif

#if defined(PPUC_NUM_LEDS_2) && defined(PPUC_LED_TYPE_2) && \
    defined(__IMXRT1062__)
DMAMEM byte frameBuffer2[PPUC_NUM_LEDS_2 * ((PPUC_LED_TYPE_2 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif

#if defined(PPUC_NUM_LEDS_3) && defined(PPUC_LED_TYPE_3) && \
    defined(__IMXRT1062__)
DMAMEM byte frameBuffer3[PPUC_NUM_LEDS_3 * ((PPUC_LED_TYPE_3 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif

#if defined(PPUC_NUM_LEDS_4) && defined(PPUC_LED_TYPE_4) && \
    defined(__IMXRT1062__)
DMAMEM byte frameBuffer4[PPUC_NUM_LEDS_4 * ((PPUC_LED_TYPE_4 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif

#if defined(PPUC_NUM_LEDS_5) && defined(PPUC_LED_TYPE_5) && \
    defined(__IMXRT1062__)
DMAMEM byte frameBuffer5[PPUC_NUM_LEDS_5 * ((PPUC_LED_TYPE_5 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif

#if defined(PPUC_NUM_LEDS_6) && defined(PPUC_LED_TYPE_6) && \
    defined(__IMXRT1062__)
DMAMEM byte frameBuffer6[PPUC_NUM_LEDS_6 * ((PPUC_LED_TYPE_6 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif

#if defined(PPUC_NUM_LEDS_7) && defined(PPUC_LED_TYPE_7) && \
    defined(__IMXRT1062__)
DMAMEM byte frameBuffer7[PPUC_NUM_LEDS_7 * ((PPUC_LED_TYPE_7 < 6) ? 3 : 4) *
                         4];  // 12 bytes per LED for RGB, 16 bytes for RGBW
#endif
 ...
 public:
  EffectsController(int ct, int pf) : EventListener() {
    ...
    if (controllerType == CONTROLLER_TEENSY_OUTPUT ||
        controllerType == CONTROLLER_TEENSY_OUTPUT_2 ||
        controllerType == CONTROLLER_PICO_OUTPUT) {
      ...
      if (controllerType != CONTROLLER_TEENSY_OUTPUT) {
        ...
        // In revision 0.1.0 these pins are D5-D7, but we don't need them for
        // the WPC GI.
        pinMode(9, INPUT);
        pinMode(10, INPUT);
        pinMode(11, INPUT);
...
#if defined(PPUC_NUM_LEDS_1) && defined(PPUC_LED_TYPE_1)
      ws2812FXDevices[0][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_1, 1, PPUC_LED_TYPE_1),
                             0, PPUC_NUM_LEDS_1 - 1, 0, 0);
      ws2812FXDevices[0][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[0] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[0] =
          new WS2812Serial(PPUC_NUM_LEDS_1, frameBuffer1,
                           ws2812FXDevices[0][0]->getWS2812FX()->getPixels(), 1,
                           PPUC_LED_TYPE_1);
      ws2812Serial[0]->begin();
      ws2812FXDevices[0][0]->getWS2812FX()->setCustomShow(
          EffectsController::ws2812SerialShow1);
#endif
...
#if defined(PPUC_NUM_LEDS_2) && defined(PPUC_LED_TYPE_2)
      ws2812FXDevices[1][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_2, 8, PPUC_LED_TYPE_2),
                             0, PPUC_NUM_LEDS_2 - 1, 0, 0);
      ws2812FXDevices[1][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[1] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[1] =
          new WS2812Serial(PPUC_NUM_LEDS_2, frameBuffer2,
                           ws2812FXDevices[1][0]->getWS2812FX()->getPixels(), 8,
                           PPUC_LED_TYPE_2);
      ws2812Serial[1]->begin();
      ws2812FXDevices[1][0]->getWS2812FX()->setCustomShow(
...
#if defined(PPUC_NUM_LEDS_3) && defined(PPUC_LED_TYPE_3)
      ws2812FXDevices[2][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_3, 14, PPUC_LED_TYPE_3),
                             0, PPUC_NUM_LEDS_3 - 1, 0, 0);
      ws2812FXDevices[2][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[2] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[2] =
          new WS2812Serial(PPUC_NUM_LEDS_3, frameBuffer3,
                           ws2812FXDevices[2][0]->getWS2812FX()->getPixels(),
                           14, PPUC_LED_TYPE_3);
      ws2812Serial[2]->begin();
      ws2812FXDevices[2][0]->getWS2812FX()->setCustomShow(
          EffectsController::ws2812SerialShow3);
#endif
...
#if defined(PPUC_NUM_LEDS_4) && defined(PPUC_LED_TYPE_4)
      ws2812FXDevices[3][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_4, 17, PPUC_LED_TYPE_4),
                             0, PPUC_NUM_LEDS_4 - 1, 0, 0);
      ws2812FXDevices[3][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[3] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[3] =
          new WS2812Serial(PPUC_NUM_LEDS_4, frameBuffer4,
                           ws2812FXDevices[3][0]->getWS2812FX()->getPixels(),
                           17, PPUC_LED_TYPE_4);
      ws2812Serial[3]->begin();
      ws2812FXDevices[3][0]->getWS2812FX()->setCustomShow(
          EffectsController::ws2812SerialShow4);
#endif
...
#if defined(PPUC_NUM_LEDS_5) && defined(PPUC_LED_TYPE_5)
      ws2812FXDevices[4][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_5, 20, PPUC_LED_TYPE_5),
                             0, PPUC_NUM_LEDS_5 - 1, 0, 0);
      ws2812FXDevices[4][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[4] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[4] =
          new WS2812Serial(PPUC_NUM_LEDS_5, frameBuffer5,
                           ws2812FXDevices[4][0]->getWS2812FX()->getPixels(),
                           20, PPUC_LED_TYPE_5);
      ws2812Serial[4]->begin();
      ws2812FXDevices[4][0]->getWS2812FX()->setCustomShow(
          EffectsController::ws2812SerialShow5);
#endif
...
#if defined(PPUC_NUM_LEDS_6) && defined(PPUC_LED_TYPE_6)
      ws2812FXDevices[5][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_6, 24, PPUC_LED_TYPE_6),
                             0, PPUC_NUM_LEDS_6 - 1, 0, 0);
      ws2812FXDevices[5][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[5] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[5] =
          new WS2812Serial(PPUC_NUM_LEDS_6, frameBuffer6,
                           ws2812FXDevices[5][0]->getWS2812FX()->getPixels(),
                           24, PPUC_LED_TYPE_6);
      ws2812Serial[5]->begin();
      ws2812FXDevices[5][0]->getWS2812FX()->setCustomShow(
...
#if defined(PPUC_NUM_LEDS_7) && defined(PPUC_LED_TYPE_7)
      ws2812FXDevices[6][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_7, 29, PPUC_LED_TYPE_7),
                             0, PPUC_NUM_LEDS_7 - 1, 0, 0);
      ws2812FXDevices[6][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[6] = 1;

#if defined(__IMXRT1062__)  // Teensy 4.1
      ws2812Serial[6] =
          new WS2812Serial(PPUC_NUM_LEDS_7, frameBuffer7,
                           ws2812FXDevices[6][0]->getWS2812FX()->getPixels(),
                           29, PPUC_LED_TYPE_7);
      ws2812Serial[6]->begin();
      ws2812FXDevices[6][0]->getWS2812FX()->setCustomShow(
          EffectsController::ws2812SerialShow7);
#endif
...
#if defined(PPUC_NUM_LEDS_1) && defined(PPUC_LED_TYPE_1)
      ws2812FXDevices[0][0] =
          new WS2812FXDevice(new WS2812FX(PPUC_NUM_LEDS_1, 6, PPUC_LED_TYPE_1),
                             0, PPUC_NUM_LEDS_1 - 1, 0, 0);
      ws2812FXDevices[0][0]->getWS2812FX()->init();
      ws2812FXDeviceCounters[0] = 1;
