...
void PwmDevices::update()
{
    ...
    for (byte i = 0; i < last; i++)
    {
        if (activated[i] > 0)
        {
            ...
            {
                // Deactivate the output if it is scheduled for delayed deactivation and the minimum pulse time is reached.
                // Deactivate the output if the maximum pulse time is reached.
            ...
            {
                // Reduce the power of the activated output if the hold power activation time pased since the activation.
...
void PwmDevices::handleEvent(Event *event)
{
    ...
    case EVENT_SOURCE_SOLENOID:
        for (byte i = 0; i < last; i++)
        {
            if ((type[i] == PWM_TYPE_SOLENOID || type[i] == PWM_TYPE_FLASHER) && number[i] == (byte)event->eventId)
            {
    ...
    case EVENT_SOURCE_SWITCH:
        // A switch event was triggered or received. Activate or deactivate any output
        // that is configured as "fastSwitch" for that switch.
        for (byte i = 0; i < last; i++)
        {
            if (type[i] == PWM_TYPE_SOLENOID && fastSwitch[i] == (byte)event->eventId)
            {
                updateSolenoidOrFlasher((bool)event->value, i);
            }
        }
