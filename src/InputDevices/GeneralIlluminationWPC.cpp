...
void GeneralIlluminationWPC::start() {
  ...
  attachInterrupt(digitalPinToInterrupt(2), GeneralIlluminationWPC::_changeD0,
                  RISING);
  attachInterrupt(digitalPinToInterrupt(3), GeneralIlluminationWPC::_changeD1,
                  RISING);
  attachInterrupt(digitalPinToInterrupt(4), GeneralIlluminationWPC::_changeD2,
                  RISING);
  attachInterrupt(digitalPinToInterrupt(5), GeneralIlluminationWPC::_changeD3,
                  RISING);
  attachInterrupt(digitalPinToInterrupt(6), GeneralIlluminationWPC::_changeD4,
                  RISING);
  attachInterrupt(digitalPinToInterrupt(12), GeneralIlluminationWPC::_changeZC,
                  FALLING);
}
  ...
  for (int string = 0; string < NUM_GI_STRINGS; string++) {
    if (sBrightness[string] != sBrightnessTarget[string]) {
      ...
      eventDispatcher->dispatch(
          new Event(EVENT_SOURCE_GI, word(0, string + 1), sBrightness[string]));
    }
  }
}
...
void GeneralIlluminationWPC::_changeD0() { giInstance->handlePinChange(0); }

void GeneralIlluminationWPC::_changeD1() { giInstance->handlePinChange(1); }

void GeneralIlluminationWPC::_changeD2() { giInstance->handlePinChange(2); }

void GeneralIlluminationWPC::_changeD3() { giInstance->handlePinChange(3); }

void GeneralIlluminationWPC::_changeD4() { giInstance->handlePinChange(4); }
...
uint8_t GeneralIlluminationWPC::dtToBrightness(uint32_t dt) {
  ...
  if (dt < (1200 - BRIGHTNESS_INTERVAL_MARGIN)) {
    // It seems that brightness level 7 is never used in any WPC pinball
    // machine. It us selectable in the GI test, but the time is not measurable.
