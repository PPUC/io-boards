...
DistributionControllerTestButtons::DistributionControllerTestButtons(
    EventDispatcher* eD) {
  eventDispatcher = eD;
  ...
  for (int i = 0; i <= 1; i++) {
    ...
    if (button[i]->pressed()) {
      eventDispatcher->dispatch(
          new Event(EVENT_SOURCE_SWITCH, word(0, 63 + i), 1));
    } else if (button[i]->released()) {
      eventDispatcher->dispatch(
          new Event(EVENT_SOURCE_SWITCH, word(0, 63 + i), 0));
    }
  }
}
