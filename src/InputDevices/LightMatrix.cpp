...
void LightMatrix::_readRow() {
  ...
  switch (inColMask) {
    ...
    default:
      ...
      // Depending on the pinball ROM version the duration of this transition
      // varies. On a Whitewater with Home ROM LH6 (contains anti ghosting
      // updates) this gap was measured to be around 30us long. Machines with
      // anti-ghosting firmware will show a gap with no column enabled for a
      // while during the transition while older firmwares might have two
      // columns enabled at the same time due to slow transistor deactivation.
      // Both cases are caught here. See also
  ...
  // waits for a number of consecutive consistent information before updating
  // the matrix.
  if (lightMatrixInstance->updateValid(inCol, inRowMask)) {
    // Serial.print(inCol + 1); Serial.print(": "); Serial.println(inRowMask,
    // BIN);
