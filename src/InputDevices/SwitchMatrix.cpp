...
void SwitchMatrix::start() {
  attachInterrupt(digitalPinToInterrupt(CS_ODD),
                  SwitchMatrix::_readRowForOddColumn, RISING);
}
...
void SwitchMatrix::readRow(int pin) {
  ...
  if (columnCounter >= 0 && columnCounter < lastColToRead) {
    ...
    if (columnCounter > 0 ||
        // The first column is only valid, if CS_X is still LOW. Otherwise it's
        // noise.
  ...
  if (pin == CS_ODD && columnCounter >= 0 && columnCounter < lastColToRead) {
    attachInterrupt(digitalPinToInterrupt(CS_EVEN),
                    SwitchMatrix::_readRowForEvenColumn, RISING);
  } else {
    attachInterrupt(digitalPinToInterrupt(CS_ODD),
                    SwitchMatrix::_readRowForOddColumn, RISING);
  }
