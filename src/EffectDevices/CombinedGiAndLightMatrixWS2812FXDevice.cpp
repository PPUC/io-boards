...
void CombinedGiAndLightMatrixWS2812FXDevice::assignLedToFlasher(uint8_t number, int16_t led, uint32_t color)
{
    ...
    for (int f = _MAX_LEDS_PER_LIGHT; f < _MAX_LEDS_PER_LIGHT + _MAX_FLASHERS; f++)
    {
...
void CombinedGiAndLightMatrixWS2812FXDevice::updateAfterGlow()
{
    ...
    for (uint8_t number = 0; number < _LIGHT_MATRIX_SIZE + _MAX_FLASHERS; number++)
    {
