...
int PUPComLink::available() { return hwSerial->available(); }

byte PUPComLink::read() { return hwSerial->read(); }

void PUPComLink::write(byte command, char msgtype, word msgindex,
                       word msgvalue) {
  // Send to PUP Com Link. But only if there's room left in write buffer.
  // Otherwise the program will be blocked. The buffer gets full if the data is
  // not fetched by PUP Com Link for any reason.
  // @todo Possible optimization to check hwSerial->availableForWrite() >= 8
